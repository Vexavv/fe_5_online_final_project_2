...
function Favorites() {
    ...
    if (!wishlist || !wishlist.products) {
        content = <span className={styles.FavoriteHeaderText}>Your wishlist is still empty :(</span>;
    } else {
        content = wishlist.products.length > 0 ? (
            <ul className={styles.FavoriteContentList}>
                    ...
                    <li className={styles.FavoriteContentListItem} key={product._id}>
                        <Link to={`/products/${product.itemNo}`}>
                            <img
                                ...
                                alt=''
                            /></Link>
                        <h2 className={styles.FavoriteContentListItemText}>{product.name}</h2>
                        <p className={styles.FavoriteContentListItemText}>${product.currentPrice}</p>
                        <div className={styles.FavoriteContentListItemButton}>
                            <HiOutlineShoppingBag className={styles.FavoriteContentListItemButtonIcon}/>
                            <FaHeart className={styles.FavoriteContentListItemButtonIcon} onClick={() => {
                ...
                <div className={styles.FavoriteContentFooter}>
                    <Button className={styles.FavoriteContentFooterBtn} text='delete wishlist'
